...
public class Tools : ITools
{
    ...
    private static string ToKey(Tuple<string, Func<ITool>> tuple) => tuple.Item1;

    private static ITool ToValue(Tuple<string, Func<ITool>> tuple) => tuple.Item2.Invoke();
    ...
    private static Option<string> FindToolPath(string executable, Option<string> specificPath) =>
        specificPath.Match(
            none: () => GetFullPathInPathVariable(executable),
            some: path => GetFullPathFromSpecificPath(executable, path));

    private static Option<string> GetFullPathFromSpecificPath(string executable, string specificPath) =>
